...
We are looking to create a large-scale dataset of robot failures in real based on failing policy executions that will be useful for various downstream applications.
In general, most types of failures are good contributions to the dataset.
However, several ways of generating failures make downstream training harder, or create redundant data.
